...
export class BallFilterService {
 ...
 activeFilterCount: Signal<number> = computed(() => {
  ...
  #filters = signal<BallFilter>({ ...this.defaultFilters });
  get filters() {
    return this.#filters;
  }
  ...
  filterBalls(balls: Ball[], filters: BallFilter): Ball[] { 
    const filteredBalls = balls.filter((ball) => {
      return (
        ...
        (filters.availability === 'all' || filters.availability === ball.availability) &&
        (filters.releaseDate === 'all' || filters.releaseDate === ball.release_date) &&
        (filters.minRg <= parseFloat(ball.core_rg) && filters.maxRg >= parseFloat(ball.core_rg)) &&
        (filters.minDiff <= parseFloat(ball.core_diff) && filters.maxDiff >= parseFloat(ball.core_diff)) &&
        (!filters.inArsenal || this.storageService.arsenal().includes(ball))
      )
    });
