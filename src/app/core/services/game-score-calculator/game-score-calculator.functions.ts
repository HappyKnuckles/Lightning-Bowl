/**
 * Functional Game Score Calculator
 * Pure functions for calculating bowling scores from frames
...
/**
 * Calculate the total score and cumulative frame scores from bowling frames
 */
export function calculateScoreFromFrames(frames: Frame[]): ScoreResult {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (i < 9) {
      ...
      if (first === 10) {
        // Strike
        const bonus = getStrikeBonus(frames, i);
        totalScore += 10 + bonus;
      } else if (first + second === 10) {
        // Spare
        const bonus = getSpareBonus(frames, i);
        totalScore += 10 + bonus;
      } else {
        // Open frame
 ...
 */
export function getStrikeBonus(frames: Frame[], frameIndex: number): number {
 ...
 */
export function getSpareBonus(frames: Frame[], frameIndex: number): number {
 ...
 */
export function calculateMaxScoreFromFrames(frames: Frame[], currentTotalScore: number): number {
  let maxScore = 300;
