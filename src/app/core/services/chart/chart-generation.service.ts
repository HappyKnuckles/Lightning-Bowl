...
export class ChartGenerationService {
  ...
  generatePatternChart(pattern: Pattern, svgElement: ElementRef, horizontal = false): void {
        // Select the SVG element and set up a viewBox for responsiveness.
        console.log('Drawing chart for pattern:', pattern);
        const svg = d3
          .select(svgElement.nativeElement)
        ...
        if (horizontal) {
          g.attr('transform', `translate(0, ${svgHeight}) rotate(-90)`);
        } else {
